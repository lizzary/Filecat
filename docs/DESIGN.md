...
| Windows  | For a **same-parent** rename: `FILE_ACTION_RENAMED_OLD_NAME` immediately followed by `FILE_ACTION_RENAMED_NEW_NAME` in the same `ReadDirectoryChangesExW` buffer (MSDN-guaranteed adjacency). For a **cross-subdirectory** move under the same subtree watch: `FILE_ACTION_REMOVED` in the source dir followed by `FILE_ACTION_ADDED` in the destination dir — `RENAMED_*` is only emitted when the two halves share a parent. | Same-parent rename surfaces as Linux does: `FILECAT_EVENT_RENAMED_OLD` then `FILECAT_EVENT_RENAMED_NEW`. Cross-subdir move surfaces as `FILECAT_EVENT_REMOVED` then `FILECAT_EVENT_CREATED`. **Either way**, both halves carry the same `event_correlation_id` (the NTFS FileId), so downstream pairs by id without caring about the event type. A delete-then-create that reuses the same MFT entry also shares the id, which lets binding-layer coalescers recognize app-level renames implemented as remove+create. |
